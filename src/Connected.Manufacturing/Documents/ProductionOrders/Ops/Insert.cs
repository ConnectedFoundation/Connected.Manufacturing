	...
	: ServiceFunction<IInsertProductionOrderDto, int>
{
	...
	{
		var entity = await storage.Open<ProductionOrder>().Update(Dto.AsEntity<ProductionOrder>(State.Add)) ?? throw new NullReferenceException(Strings.ErrEntityExpected);
